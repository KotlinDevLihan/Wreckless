...
fn magic_index(occupancies: Bitboard, entry: &MagicEntry) -> u32 {
    // SAFETY: the build script sets `use_pext` only when the target enables
    // BMI2 (and the attack maps were generated in pext order to match).
    unsafe { std::arch::x86_64::_pext_u64(occupancies.0, entry.mask) as u32 + entry.offset }
}
