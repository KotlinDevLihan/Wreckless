...
pub struct ThreadData {
    ...
    /// Thread-local, deliberately -- see [`PawnHistory`] for why this one is not
    /// pooled with the correction histories it used to live beside.
...
impl ThreadData {
    ...
    pub fn print_uci_info(&mut self, depth: i32) {
        ...
        for pv_index in 0..self.multi_pv {
            ...
            if self.shared.show_wdl.load(Ordering::Relaxed) {
                let (win, draw, loss) = wdl_model(score, &self.board);
                formatted_score.push_str(&format!(" wdl {win} {draw} {loss}"));
            }
