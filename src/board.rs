...
impl Board {
    ...
    pub fn remove_piece(&mut self, square: Square) -> Piece {
        ...
        // Toggled before the bitboard is cleared, so `popcount()` here still
        // includes the piece being removed -- the same count value used when
        // it was added, so the XOR cancels out (see `toggle_material`).
        self.state.keys.toggle_material(piece, self.colored_pieces(piece.color(), piece.piece_type()).popcount());
    ...
    pub fn update_threats(&mut self) {
        ...
        // BOTH colours. An earlier revision computed pins for the side to move
        // only, on the grounds that the sole consumers are `pinned(stm)` in
        // movegen and `pinned(stm)`/`pinners(!stm)` in `see()`, both of which
        // come out of the `stm` pass. That is wrong about `see()`: its `stm` is
        // a *local* that starts at `!side_to_move()` and flips on every
        // exchange step, so across the loop it reads both colours' pins. The
        // very first iteration reads `pinners(side_to_move())` and
        // `pinned(!side_to_move())` -- precisely the two the optimisation
        // zeroed -- so the pinned-defender exclusion silently never fired.
        //
        // The effect is that SEE credits a pinned defender with being able to
        // recapture, undervalues the capture, and discards winning captures.
        // SEE feeds both move ordering and pruning, so the cost is diffuse.
